...
**Architecture:** Laravel keeps `/up` for process liveness. A new controller delegates to a small readiness service that checks configured required dependencies without side effects and returns only a status token. GSeven's split database configuration is probed through actual read- and write-path queries. A dedicated service provider registers the endpoint outside stateful middleware groups. GitHub Actions validates existing PHP and JavaScript contracts; the organization scheduler remains the sole PR mutation authority.
...
4. Add unit tests for database manager, write-database, read-database, cache, storage, exception, unknown-check, empty-check, malformed-check, and malformed-storage-path branches.
...
5. Execute `SELECT 1` against the write database path and then the read database path so a stale connection object cannot produce a false-ready result.
...
5. Explain that the `database` check verifies both split read and write query paths.
6. Add APA 7th references for HTTP semantics/caching, Kubernetes probes, Laravel health routing, NIST SSDF, and SLSA 1.2.
7. Record a customer-action-oriented change fragment. Fold it into the root `CHANGELOG.md` only when exact-head release checks pass and a release version is actually cut; do not rewrite released history during an unverified feature PR.
